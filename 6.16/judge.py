...
class judge:
    ...
    Test_Unitsize = Unitsize-int(Unitsize*Trainsize)
    ...
    def __init__(self,pos_test,pos_real):    #初始化参数，基本评定中包括每个点的均值方差（二维）以及理应所在位置，然后还要有个点阵的总偏差
        ...
        for i in range(0,judge.Squaresize):
            ...
            Xtemp = np.zeros(judge.Test_Unitsize)
            Ytemp = np.zeros(judge.Test_Unitsize)
            for  j in range(0,judge.Test_Unitsize):
    ...
    def save_file(self,path,filename):
        ...
        bias_data = np.array([[self.xbias,self.ybias,self.bias,self.sigma]])
        data2 = pd.DataFrame(bias_data)
        data2.columns = ['xbias','ybias','bias','sigma']
