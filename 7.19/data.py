...
for current_i in range(0,int(Unitsize*Trainsize)):
    for m in range(0,Ysize):
        for n in range(0,Xsize):#三个循环地位上等价，处理和保存数据都在三个循环内部
            ...
            relation_train[number_train][0] = Ybegin + Gapsize*m
            relation_train[number_train][1] = Xbegin + Gapsize*n
...
for current_i in range(int(Unitsize*Trainsize),Unitsize):
    for m in range(0,Ysize):
        for n in range(0,Xsize):#三个循环地位上等价，处理和保存数据都在三个循环内部
            ...
            relation_test[number_test][0] = Ybegin + Gapsize*m 
            relation_test[number_test][1] = Xbegin + Gapsize*n
    ...
    if int(10*(current_i+1-int(Unitsize*Trainsize))/(Unitsize-int(Unitsize*Trainsize)))>count:
        count+=1
        if count == 10:
