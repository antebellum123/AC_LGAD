...
class judge:
    ...
    def __init__(self,pos_test,pos_real):    #初始化参数，基本评定中包括每个点的均值方差（二维）以及理应所在位置，然后还要有个点阵的总偏差
        ...
        for i in range(0,len(pos_real)):
            SSE+= (self.pos_real[i][0]-self.pos_test[i][0])**2+(self.pos_real[i][1]-self.pos_test[i][1])**2
        ...
        self.matrix_sigma_total= (Xpos_sigma**2+Ypos_sigma**2)**0.5
        self.RMSE = (SSE/len(pos_test))**0.5
        self.sigma = np.mean(self.matrix_sigma_total)
    ...
    def save_file(self,path,filename):
        ...
        matrix = np.hstack((self.matrix_real,self.matrix_miu,self.matrix_sigma,self.matrix_sigma_total))
        data1 = pd.DataFrame(matrix)
        data1.columns = ['X','Y','Xmiu','Ymiu','Xsigma','Ysigma','Sigma']
        bias_data = np.array([[self.RMSE,self.sigma,self.xbias,self.ybias,self.bias,self.in_xbias,self.in_ybias,self.in_bias]])
        data2 = pd.DataFrame(bias_data)
        data2.columns = ['RMSE','sigma_mu','xbias_sigma','ybias_sigma','bias_sigma','in_xbias_sigma','in_ybias_sigma','in_bias_sigma']
