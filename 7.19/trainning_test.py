...
isExists=os.path.exists(curr_path+'\trainning_test')
if isExists:
    shutil.rmtree(curr_path+'\trainning_test')  
    os.mkdir(curr_path+'\trainning_test') #清空数据
...
#c = np.hstack((data_testx_lin,data_posx))

'''
qx = np.arange(-0.2,0.3,0.001).reshape(-1,1)
qy = np.arange(-0.2,0.3,0.001).reshape(-1,1)
test_np = np.hstack((qx,qy))
pos_lin=lin.predict(test_np)

plt.plot(qx,pos_lin[:,0],'r')
plt.scatter(data_testx_lin[:,0],data_posx[:,0])
plt.xlabel('Qx')
plt.ylabel('X')
'''
...
path = curr_path+'\\trainning_test'
class_lin.save_file(path,'class_lin')
class_nlin.save_file(path,'class_nlin')
...
class_MLP.save_file(path,'class_MLP')
